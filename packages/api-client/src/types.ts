...
export interface ApiClientOptions {
  ...
  /**
   * Идентификатор этого терминала из его регистрации. Уходит заголовком
   * `X-Terminal-Id` на каждом запросе: узел определяет по нему заведение
   * и тип терминала, а значит и набор экранов.
   */
  terminalId?: UUID;
  /**
   * Заведение текущей сессии — заголовок `X-Venue-Id`. Геттером, потому что
   * до входа оно неизвестно, а клиент создаётся раньше.
   */
  getVenueId?: () => UUID | null;
